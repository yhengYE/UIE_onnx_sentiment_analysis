...
def parse_args():
    ...
    # FP16模型: 在使用GPU 且设置`use_fp16 = True `时，加载并转换`inference.onnx` '为 FP16 模型(`fp16 model.onnx )。
    # FP32模型: 在其他情况下，直接使用、inference.onnx`。


    parser.add_argument(
        "--max_seq_len",
        default=512000000000000,
        type=int,
    ...
    parser.add_argument(
        ...
        default="onnx",#不要改，要用pytorch的话得添加转换文件pytorch_model.bin
        help="Select which engine to run model, defaults to pytorch."
    )
    parser.add_argument('--file_path', type=str,default='input.txt', help="Input text file")
    parser.add_argument('--save_path', type=str, default='output.json', help="Output JSON file")
    # parser.add_argument("--schema", default="[{'评价维度': ['观点词', '情感倾向[正向,负向,未提及]']}]",help="The schema for UIE to extract infomation.")
    # parser.add_argument("--schema_lang", default="ch", type=str, help="language")
    # 很神奇
...
def write_json_file(examples, save_path):
        with open(save_path, "w", encoding="utf-8") as f:
            for example in examples:
                json.dump(example, f, ensure_ascii=False, cls=NumpyEncoder)
                f.write('\n')  # 每个 JSON 对象写入完毕后换行
#这个是在gui页面运行的，不用管
def run_analysis(file_path, save_path):
    args = parse_args()
    args.schema = [{'评价维度': ['观点词', '情感倾向[正向,负向,未提及]']}]
    args.schema_lang = "ch"
    ...
    with open(file_path, 'r', encoding='utf-8') as infile:
        for line in infile:
            ...
            if input_text:
                prediction = uie([input_text])
                if isinstance(prediction, list) and len(prediction) > 0 and isinstance(prediction[0], dict):
                    results.append(prediction[0])
                else:
                    print(f"Unexpected prediction format for input: {input_text} - {prediction}")

    write_json_file(results, save_path)

if __name__ == '__main__':
    run_analysis('input.txt', 'output.json')